...
//CN changes
//uint8_t active_extruders[2] = {0, 1};
...
// CN changes
//void tool_changes(const uint8_t tmp_extruders[], const float fr_mm_m=0.0, bool no_move=false);
...
bool get_target_extruder_from_command(int code) {
 ...
 //   memcpy(target_extruders, active_extruders, sizeof(target_extruders));
...
/**
 * CN changes
 * move both extruders simultaneously
 */
/*
inline void gcode_G93() {
  if (IsRunning()) {
    gcode_t0_destination();
    gcode_t1_destination();
...
inline void gcode_M104() {
  ...
  if (code_seen('S')) {
    // CN changes:
    //for (int i = 0; i < 2; i ++) {
      //thermalManager.setTargetHotend(code_value_temp_abs(), target_extruders[i]);
    //}
...
inline void gcode_M503() {
  ...
  inline void gcode_M605() {
    ...
    if (extruder_duplication_enabled) {
      extruder_duplication_enabled = false;
      SERIAL_ECHO_START;
      SERIAL_ECHO("duplication is disabled");
      SERIAL_EOL;
    }
    else {
      SERIAL_ECHO_START;
      SERIAL_ECHO("duplication is enabled");
      SERIAL_EOL;
...
//    #if HOTENDS > 1
//
//      if (sizeof(tmp_extruders) / sizeof(uint8_t) >= EXTRUDERS) {
//        invalid_extruder_error(tmp_extruders[(sizeof(tmp_extruders) / sizeof(uint8_t)) - 1]);
//        return;
//      }
//
//      float old_feedrate_mm_m = feedrate_mm_m;
...
//      feedrate_mm_m = fr_mm_m > 0.0 ? (old_feedrate_mm_m = fr_mm_m) : XY_PROBE_FEEDRATE_MM_M;
//
//      // Set the new active extruders
//      memcpy(active_extruders, tmp_extruders, sizeof(active_extruders));
//  #endif
//}
...
/** CN changes
 * Activate multiple extruders at the same time
 */
//inline void gcode_Ts(uint8_t tmp_extruders[]) {
//
//
//  #if HOTENDS == 1 || (ENABLED(MIXING_EXTRUDER) && MIXING_VIRTUAL_TOOLS > 1)
//
//    tool_changes(tmp_extruders);
//
//  #elif HOTENDS > 1
//
//    tool_changes(
//      tmp_extruders,
//      code_seen('F') ? code_value_axis_units(X_AXIS) : 0.0,
//      (tmp_extruders == active_extruders) || (code_seen('S') && code_value_bool())
//    );
//
//  #endif
//
...
void prepare_move_to_destinations() {
